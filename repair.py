...
# 1510행의st.markdown(..., unsafe_allow_html=True) 끝난 직후, st.markdown('📊 Dynamic Stock Backtester'...) 이전 지점 탐색
target_routing_split = """                            for diag in diagnostics:
                                st.markdown(f\"\"\"
...
idx = content.find(target_routing_split)

if idx != -1:
    # 매칭 지점 바로 다음 위치 계산
    insert_point = idx + len(target_routing_split)
    
    # 🚨 시장 위험 지표 대시보드 탭 로직을 삽입
    risk_dashboard_code = """
    ...
    # =======================================================
        ...
        with st.spinner("글로벌 거시 경제 및 풋/콜 옵션 실시간 수치 다운로드 및 분석 중..."):
            ...
            # SPY 옵션 데이터를 통한 실시간 풋/콜 미결제약정 비율 (Put/Call Ratio) 프록시 연산
            try:
            ...
            # (2) Fear & Greed Index (F&G Index Proxy)
            # VIX, 환율, 신용도 등을 결합하여 0~100 점수화
...
"""
    ...
    # app.py 구조 재조립
    content = content[:insert_point] + risk_dashboard_code + content[insert_point:]
    
    with open(file_path, 'w', encoding='utf-8') as f:
        f.write(content)
        
    print("Routing repair executed successfully.")
else:
    print("Target routing split pattern not found.")
