                ...
                with st.spinner("실시간 시장 가격을 야후 파이낸스(yfinance)로부터 수집 중..."):
                    ...
                    if has_error:
                        ...
                        with c_div2:
                            ...
                            cushion_tickers = ["TLT", "IEF", "SHY", "SCHD", "JEPI", "O", "GLD", "IAU", "PDBC", "DBC"]
                            cushion_sum = 0.0
                            for _, r in df_res.iterrows():
                                ticker_upper = r["티커"].upper()
                                if any(x in ticker_upper for x in cushion_tickers) or "채권" in ticker_upper or "현금" in ticker_upper:
                                    cushion_sum += r["평가금액"]
                                    
                            cushion_pct = (cushion_sum / total_eval_value) * 100
                            
                            if cushion_pct >= 20.0:
                                cushion_status = "🟢 방어막 우수 (탄탄한 완충 지대)"
                                cushion_desc = f"전체 자산 중 **{cushion_pct:.1f}%**가 채권, 금, 고배당 등 변동성 완충 및 리밸런싱 예비 자산으로 구성되어 있습니다. 마켓 크래시 시 자산 방어를 든든히 수행하며 저가 매수 탄환 역할을 할 수 있습니다."
                                cushion_color = "#10B981"
                            elif cushion_pct >= 5.0:
                                cushion_status = "🟡 방어막 보통 (최소 완충 지대 확보)"
                                cushion_desc = f"전체 자산 중 **{cushion_pct:.1f}%**가 안전/배당성 자산에 배정되어 있어 최소한의 안전지대를 갖췄습니다. 주식형 쏠림에 대응해 비중을 15% 이상으로 증대시키는 것을 권장합니다."
                                cushion_color = "#F1C40F"
                            else:
                                cushion_status = "🚨 안전 방어막 부재 (변동성 전면 노출)"
                                cushion_desc = f"전체 자산 중 안전/완충 자산 비중이 **{cushion_pct:.1f}%** 미만으로 극도로 미미합니다. 시장 급락 시 가용한 현금 흐름 및 리밸런싱 여력이 없으므로 채권(TLT), 배당(SCHD) 또는 현금 비중을 확보할 것을 조언합니다."
