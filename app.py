...
# 커스텀 CSS로 디자인 개선 (유려한 글꼴, 그라데이션 및 카드 스타일)
st.markdown("""
    <style>
...
def calculate_metrics(data, initial_capital):
    ...
    # 영업일 수 계산 및 연도 변환 (대략 252 거래일을 1년으로 산정)
    total_days = len(data)
    ...
    if short_window >= long_window:
        ...
        with st.spinner("주가 데이터를 가져오고 백테스트를 진행 중입니다..."):
            ...
            if df is None or len(df) < long_window:
                ...
                # 2개의 행으로 이루어진 서브플롯 구성 (행 1: 주가 및 이평선 / 행 2: 누적 자산가치 비교)
                fig = make_subplots(
                    rows=2, cols=1, 
                    shared_xaxes=True, 
                ...
                # 4. 누적 포트폴리오 가치 추가 (전략 vs Buy & Hold)
                fig.add_trace(
